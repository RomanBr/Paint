...
var worker = new window.SharedWorker("worker.js");
...
var fName = "3.bmp";
		...
		/*if (Notification.permission !== 'denied') {
			Notification.requestPermission(function (permission) {
			  ...
			  if (permission === "granted") {
				var n = new Notification("<a>Hi there!</a>");
	...
	function execAlg2 (e) {
		...
		try {
			...
			worker.port.postMessage ([d, w, h, f, data2, data3, e.effect, e.options]);
		}
	...
	function execAlg3 (e) {
		...
		try {
			...
			xmlhttp.send(JSON.stringify({'name': data3 }));
		}
		catch (e) { console.log (e); }
	}
	...
	worker.port.onmessage = function (e) {
		data2 = e.data[0];
		var imageData2 = new ImageData(data2, w, h);
		ctx.putImageData(imageData2, 0, 0);
	    var diff = new Date() - start;
        log.innerHTML = "Process done in " + diff + " ms (no web workers)";
		console.log("OK");
